...
// two Direction_E2 will take the shorter path. (But notice that we
// constraint Segment_S1 to always be smaller than half a circle.)
...
interpolate(const Bbox_E2<NT>& from, const Bbox_E2<NT>& to)
{
    ...
    Transformation_E2<NT> scale(SCALE, Vector_E2<NT>( to.width()  / from.width(),
...
{
    const NT scene_AR = scene.width() / scene.height();
    const NT window_AR = window.width() / window.height();
