...
public:
    MyWidget(QWidget * parent = 0)
        : QWidget(parent),
          viewer_min( Point_E3d(0, 0, 2.5 + epsilon) ),
          viewer_max( Point_E3d(0, 0, 0.0 + epsilon) ),
          i(0),
          total(50),
          fov(80)
    {
    ...
    void render(QPainter & painter)
    {
        ...
        while( pit != ifs.polygons.end() )
        {
            ...
            while( vit != pit->vertex_indices.end() ) {
                ...
                std::pair<bool, Segment_E3d> p = my_pipeline( Segment_E3d( source, target ) );

                if( p.first ) {
                    QPointF qpf_src = to_QPointF( my_projection(p.second.source()));
                    QPointF qpf_tgt = to_QPointF( my_projection(p.second.target()));
                    painter.drawLine( qpf_src, qpf_tgt );
                }
